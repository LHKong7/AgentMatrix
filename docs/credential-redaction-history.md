...
`prepareRunLaunch` resolves current references, builds the current environment, and merges raw/native-encoded values into the history. It returns merged values for masking and argument checks, never as fallback authentication. OpenCode additionally retains ephemeral observer authentication forms before starting ACP; its preflight already uses the earlier history. Stderr, journaled user messages, normalized assistant/reasoning/tool output and supported interaction text use the known-value redactors.

The merged values also guard captured command-line arguments before attachment preflight. Raw, JSON/URL and recorded native forms are rejected with a static configuration error instead of being passed to a process or rewritten. Current authentication still resolves only the captured references. The [argument-boundary audit](credential-data-boundaries.md#attachment-command-line-arguments) records current/retired/legacy cases and the separate limits for saved ordinary text and installation probes.
...
## Live events and successful IPC responses

The [September 20 desktop result](probes/2026-09-20-stream-boundary-desktop.json) strengthens the same four-route fixture with distinct raw, JSON-escaped and URL-encoded credential echoes. Its synthetic credentials contain quotes, a slash and a brace, so those forms differ. The local provider emits each primary response in 13-character content fragments. A resumed response repeats the earlier credential as well as the current one. Submitted user messages also include these forms; the provider verifies receipt of the original input while the application's recorded user event must be masked.

Every turn registers a real preload subscription before sending. The fixture checks every delivered event, joins assistant deltas to detect exposure across event boundaries, and compares the complete delivery sequence with `readEvents` after the turn. It also scans history pages, exported JSONL, bilingual configuration dialogs, and successful responses from commands, snapshot reads, subscriptions, configuration reports, credential writes/status, workspace reads, engine probes, application information, session lists, unused-data/pending-removal lists, impact previews and export receipts. The checker enumerates fixture encodings independently of the production redactor.

All **20 turns, 161 delivered events, 568 successful-response checks and 112 application regular-file scans** pass. All eight masking histories are removed with their captures. The observed Electron stderr contains 429 bytes without a fixture credential; stdout and renderer console emit no messages in this run. Output capture continues across all three Electron launches and final shutdown. These observations do not establish coverage of every operating-system log or every possible exception. Native state directories remain excluded, and the fixture still verifies that native conversation context retains an earlier plaintext echo.

Provider fragmentation is not a promise about native event granularity. OpenCode and Pi delivered multiple message events in this run; DSH commonly published a committed message in one event. Both DSH routes still pass exact native restoration, current-key authentication and historical masking. Successful-payload checks cover these fixture configurations, not arbitrary credentials manually copied into unrelated names or saved content.

Production source, unit tests and the application dependency declarations are unchanged from `7bcdb19`; its **987-test / 61-file** result remains applicable and was not rerun. The modified fixture passes ESLint and Node syntax checking and ran against the existing built application, without an overlapping native suite or rebuild. The missing temporary Pi/DSH installations were restored from the local npm cache at **0.85.1 / 0.1.5-rc.2**, with lifecycle scripts disabled. [Validation metadata](probes/2026-09-20-stream-boundary-validation.json) records the source trees, observed build/entry hashes and restored dependency-lock digest. This remains local-provider evidence and does not pass the full X3 or external-provider gates.
