...
For OpenCode, the fixture additionally captures a native Markdown Prompt override before starting a new session. Actual native readback rejects the Prompt field; the UI identifies it without showing the native body or inventing a successful observation. The native file remains unchanged. Optional `AGENT_MATRIX_DIAGNOSTIC_SCREENSHOT` captures report views with `.en.png` and `.zh.png`, plus OpenCode `.json`, `.markdown` and `.instance` scenario views in both languages.
...
New OpenCode captures also use `opencode-instance-config` for mismatches or unavailable readback from the owned ACP server. The [instance check](opencode-instance-configuration.md) applies before Ready, on resume and around turns, and uses the same safe field classifier as preflight. This identifies a current-server conflict, but does not infer its winning source file.

OpenCode mismatch receipts can now retain [matching literal source declarations](opencode-override-sources.md). Captured source indices and affected field groups survive journal restart; reports display historical paths without values or fresh reads. Multiple matches remain ambiguous, while changed, dynamic and unsupported sources remain unassigned. This adds useful source evidence without asserting a complete native winning-source trace.
