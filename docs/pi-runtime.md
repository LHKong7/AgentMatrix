...
These dialog mappings do not establish universal tool approvals. The adapter disables unselected extensions and rejects MCP bindings and the shared `ask` execution policy. Without plugins it accepts no-tools or unrestricted execution, with project trust configured separately. [Selected Pi extensions](pi-plugin-activation.md) require unrestricted execution and verified native instance receipts. Their commands and handled input can complete without a model run; the adapter checks successful handling after native prompt acceptance. Session-changing commands still fail identity checks. Internal witness commands are blocked.
...
Pi may defer creating its native session file until a model turn writes history. A verified live session can handle extension-only commands before this file exists, but an empty or extension-only conversation is not guaranteed to resume. Once native history exists, its deletion is an error; resume always requires the native header. Failed restoration preserves the application history and does not create a replacement conversation. Known-secret redaction applies to AgentMatrix journals and UI, not to Pi's engine-owned transcript files.
...
The [selected-extension fixtures](pi-plugin-activation.md) additionally cover native factory and startup checks, custom tools/system hooks, extension-only commands and handled input, confirmation cancellation, and bilingual desktop interactions.
