...
3. Save an enabled Agent profile with that installation and model. Set a default working directory or choose one when starting a session. Select shared Prompt, Skill, and MCP bindings as needed. Resolve missing configuration and review the engine-specific settings. Selected installed [OpenCode ESM plugins](opencode-plugin-activation.md), [Pi extensions](pi-plugin-activation.md), and [DSH modules](dsh-plugin-activation.md) have startup/resume checks; DSH also supports per-instance JSON configuration. Pi rejects MCP bindings and the shared `ask` execution policy. Select no-tools or unrestricted tools explicitly; Pi extensions require unrestricted execution. Project trust is a separate Pi option.
4. Open **Sessions** and choose the saved profile. Review **Working directory**, enter an absolute path or use **Choose folder**, then start a new session. The main process validates the directory, captures immutable inputs, resolves credentials, checks the native version/configuration, and connects the native protocol. The conversation becomes ready only after the applicable native model, agent or Skill settings, and session identity pass verification.
...
The launch directory is a temporary override for new sessions. It does not edit the saved profile. **Use profile default**, switching profiles, leaving the page, or reloading clears the override; cancelling the folder chooser preserves the previous selection. The compatibility preview resolves with the chosen directory, so a profile without a saved default can launch after a directory is selected. Empty or relative paths cannot start; missing paths and regular files are rejected by the main process before native-source inspection or capture. Folder selection itself does not execute a CLI. A symlink is resolved to its canonical directory, while a typed alias is retained in the captured request and rechecked on reuse.

The chosen directory controls native-source inspection, process cwd, and the session's captured identity. It is not a filesystem sandbox. Existing conversations, including **Start session** for an already captured conversation and **Resume session**, retain their original directory regardless of the new-session field. The conversation header and configuration report show that captured directory. Reports compare with the saved profile default, so an intentional launch override can appear as an execution difference even without a library edit.

A complete shared preview is only a planned configuration, not proof of applied native settings. OpenCode project/global/managed configuration remains relevant; readback conflicts stop startup. Pi uses a private native home and explicit project trust/context settings, with observed sources checked before reuse. DSH captures its native composition, isolates controls, and checks its model/session identity; it has no ACP permission-policy readback. See the [Pi configuration boundaries](pi-configuration.md) and [DSH runtime boundaries](dsh-runtime.md).
...
Each session retains the configuration captured at creation. Editing a shared Prompt or other asset affects new sessions; it does not rewrite an existing session's inputs. Resume verifies the captured inputs and observed native sources before restoring the same native conversation. Incompatible or failed restoration is reported without creating a replacement conversation. Pi may not persist an empty native conversation until a model turn writes history, so an empty or extension-only Pi conversation is not guaranteed to resume.

Switching pages or reloading the renderer attaches to durable history and current events without resubmitting messages. The live UI retains up to 1,000 recent events / approximately 4 MiB and shows a notice when history is capped. **Session history** browses the full saved journal in bounded pages and exports all events through a selected cursor to JSONL through the native Save dialog. Historical requests are read-only. See [history browsing and export](session-history.md) for page boundaries, export semantics, and limits. Closed conversations can be explicitly [deleted with reference-checked capture cleanup](session-retention.md); failed cleanup remains retriable. **Unused run data** separately reviews and removes unassociated captures and abandoned stages; [its cleanup policy](unused-run-data.md) preserves retained references and supports confirmed retry. Native output is rendered as literal text, including HTML-like content. Tool truncation and unknown usage/cost remain explicit.
...
The launch-directory extension passes the installed [OpenCode](probes/2026-09-18-working-directory-opencode.json), [Pi](probes/2026-09-18-working-directory-pi.json), and [DSH](probes/2026-09-18-working-directory-dsh.json) desktop fixtures on macOS arm64. Each fixture rejects empty/relative/missing paths, cancels the chooser without changing the field, resets overrides on profile selection, and launches a second conversation in a different folder containing Unicode and spaces. Native relative file reads return that folder's distinct marker, while the saved profile and first conversation retain their default directory. After app restart, native resume preserves the second conversation's directory, native ID, input digest, and prior tool context even when the new-session field points to a missing directory. Both languages and the configuration report are checked. Only the OS chooser is substituted; the main-process picker IPC, host filesystem validation, factory, native CLI, and persistence run unchanged. Optional `AGENT_MATRIX_DIRECTORY_SCREENSHOT` records English and Chinese screenshots. Unit tests additionally cover regular files, host-invalid paths, symlink resolution, profiles with empty defaults, selected-project source capture, and refusal to reuse changed native sources.

The [lower-level OpenCode fixture](opencode-configuration.md) additionally covers directory Skills and stdio MCP mapping; the [Pi configuration fixture](pi-configuration.md) covers directory Skills and reference files. These results do not pass the external-provider or three-engine delivery gates. The intended endpoint/model/auth route, library-wide update impact reporting and complete native provenance, other platforms, remote MCP/OAuth, and broader plugin/bundle coverage remain open. The [OpenCode plugin desktop result](probes/2026-09-18-opencode-plugin-desktop.json) additionally verifies selected ESM plugin hooks, instance checks, bilingual reports, and native resume.

The [Pi extension desktop result](probes/2026-09-18-pi-plugin-desktop.json) additionally verifies selected extension hooks, current-session activation checks, English/Chinese confirmation controls, commands and handled input without model calls, and native resume.

The [DSH plugin desktop result](probes/2026-09-18-dsh-plugin-desktop.json) additionally verifies English/Chinese per-instance JSON editing and persistence, selected native system sections reaching the local provider, activation reports, and fresh evidence after restart/resume. This fixture uses an appended shared prompt so the plugin section remains visible alongside shared instructions.
