...
3. Save an enabled Agent profile with that installation and model. Set a default working directory or choose one when starting a session. Select shared Prompt, Skill, and MCP bindings as needed. Resolve missing configuration and review the engine-specific settings. Selected installed [OpenCode ESM plugins](opencode-plugin-activation.md) and [Pi extensions](pi-plugin-activation.md) have startup/resume checks; DSH selected plugins remain unsupported. Pi rejects MCP bindings and the shared `ask` execution policy. Select no-tools or unrestricted tools explicitly; Pi extensions require unrestricted execution. Project trust is a separate Pi option.
