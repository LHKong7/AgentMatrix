...
Switching pages or reloading the renderer attaches to durable history and current events without resubmitting messages. The live UI retains up to 1,000 recent events / approximately 4 MiB and shows a notice when history is capped. **Session history** browses the full saved journal in bounded pages and exports all events through a selected cursor to JSONL through the native Save dialog. Historical requests are read-only. See [history browsing and export](session-history.md) for page boundaries, export semantics, and limits. Native output is rendered as literal text, including HTML-like content. Tool truncation and unknown usage/cost remain explicit.
