...
3. Save an enabled Agent profile with that installation and model. Set a default working directory or choose one when starting a session. Select shared Prompt, Skill, and MCP bindings as needed. Resolve missing configuration and review the engine-specific settings. Native plugins are not supported by these adapter revisions. Pi additionally rejects MCP bindings and the shared `ask` execution policy. Select no-tools or unrestricted tools explicitly; project trust is a separate Pi option.
