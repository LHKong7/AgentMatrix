...
**Implementation evidence:** D1 configuration and the D2/D3 runtime/desktop path now pass local macOS fixtures; see [DSH runtime evidence](dsh-runtime.md). Both provider components are verified through the production factory/coordinator. ACP has no effective permission-policy readback: captured composition checks and native behavioral fixtures provide the current policy evidence. D4 remains open for its full acceptance requirements.
