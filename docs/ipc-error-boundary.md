...
The fixture now includes six [credential name/ID rejection checks](credential-data-boundaries.md#credential-names-and-ids), bringing the total to **20 rejected operations**. Both locales exercise rejected name creation/replacement in the actual credential form and rejected IDs through preload. Original vault bytes and metadata remain unchanged, existing secrets still decrypt, and four corrected form saves recover with real OS encryption. The [desktop result](probes/2026-09-20-credential-metadata-desktop.json) records this additional boundary; the earlier 14 cases still pass.
