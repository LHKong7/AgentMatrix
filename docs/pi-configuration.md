...
The current adapter checks both native Skill command names and `sourceInfo.path` against the captured mapping, including generated wrappers for plain Markdown. Matching a name from a different directory fails before Ready or before the next submitted turn; a shadowing extension/prompt command also fails. These [source checks](skill-source-verification.md) are separate from invocation evidence and retain their own durable check identifier.
...
Chat Completions has native provider-call evidence in this fixture. The separate [Anthropic Messages fixture](anthropic-provider-acceptance.md) adds actual endpoint/header, Prompt, tool, cancellation, native-resume and failure evidence. Responses and Gemini have mapping tests only. New Anthropic captures record the generated native base for readback; legacy captures retain their literal endpoint semantics. OAuth, keyless routes, arbitrary authentication headers, and other APIs fail with a configuration diagnostic. No API translation is performed. Model context limits and cost metadata currently use Pi's native custom-model defaults; these defaults are not verified model capacity or billable pricing. The runtime leaves unverified cost unknown.
...
Discovered extensions, prompt templates, themes, and Skills are disabled. Explicitly selected shared Skills remain loadable with `--no-skills --skill <entry>`, which is verified on the pinned installation. `--offline` disables native background update behavior; it does not prevent the explicitly configured model request. [Selected installed extensions](pi-plugin-activation.md) now load explicitly alongside disabled ambient discovery, with source and native session checks. These bindings require unrestricted execution. MCP bindings remain unsupported until a compatible MCP extension has separate acceptance evidence.
