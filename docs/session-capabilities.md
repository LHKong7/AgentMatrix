...
[DSH MCP initialization receipts](dsh-mcp-startup.md) now identify completed native startup in per-service rows. They never pass `mcp-connectivity`: native registrations can survive failed service access. Source-discriminated schemas and report identity checks preserve this distinction across startup, restart and resume.
