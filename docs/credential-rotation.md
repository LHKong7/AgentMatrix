...
The vault returns plaintext and version metadata in the same serialized operation. A replacement queued during decryption cannot associate the old plaintext with the new revision. The launch tracker deduplicates references, accepts only captured references, and requires complete resolution. More than 256 distinct references fail before launch. Plaintext remains in the existing in-memory launch/redaction path.
