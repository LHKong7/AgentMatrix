...
`shutdown()` rejects new commands, aborts and disposes owned attachments, and records resumable interruptions. It has been tested directly, including against OpenCode; Electron quit/window lifecycle hooks are still separate work. Native resume receives the original snapshot/native ID and a new run ID. An adapter returning a different native ID fails and is disposed.
