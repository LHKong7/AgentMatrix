...
Durable reads paginate the full transcript independently of the recent-event buffer. Records are bounded to 1 MiB and a journal defaults to 256 MiB, with reserved space for terminal events and restart recovery. Hitting the limit rejects additional content and preserves existing history. Native resume, recent history, and a separate read-only historical view are exposed in the desktop UI. The historical API reads forward/backward pages through a fixed inclusive cursor, with count and byte bounds; export streams that complete prefix into an atomic JSONL file selected through the native Save dialog. Both omit command receipts and preserve stored redaction. See [history browsing and export](session-history.md). Retention controls and automatic deletion remain open.
...
The Ready event may include a bounded set of adapter check identifiers. State projection records them with the process run, native conversation, input digest, and timestamp. The field is optional for compatibility with existing journals. Resume replaces evidence only after successful native acknowledgment; old observations remain historical while interrupted or resuming. `sessions.configuration` uses the same sender checks and validated session identity as other session queries. It returns an allowlisted report, without resolving credentials or exposing raw native configuration. See [configuration reports](configuration-report.md).
