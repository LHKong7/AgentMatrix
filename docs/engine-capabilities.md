...
This implements static compatibility diagnostics and the descriptor foundation of A8/A11/C3/D3. It does not complete A8 runtime availability or model-service acceptance. Recorded native checks remain in the separate [session configuration report](configuration-report.md); promoting those scoped observations into reusable capability descriptors is still open. OpenCode ESM plugin bindings are eligible for separate startup checks; pure mode blocks them. Selected Pi extensions require unrestricted execution and separate startup checks; prefix arguments cannot add untracked extensions. Selected DSH modules are eligible for native boot/fiber/session checks and bounded per-instance JSON configuration; options for a different engine are diagnosed before launch. See [DSH activation](dsh-plugin-activation.md). Credential-value rotation, complete native provenance/import, broader plugin coverage, external-provider acceptance, and cross-platform acceptance remain open. Windows is blocked by the current desktop runtime path; Linux remains unverified by these macOS fixtures.
