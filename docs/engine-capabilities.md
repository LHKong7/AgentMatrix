...
| ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| All three        | Wrong engine/version/mode, incompatible host platform, unsupported provider/authentication route, missing key reference, conflicting replacements, invalid command values                              |
...
This implements static compatibility diagnostics and the descriptor foundation of A8/A11/C3/D3. [Native capability evidence](session-capabilities.md) now separately aggregates eighteen scoped capabilities from durable attachment observations. Runtime descriptors belong to a captured session, not the saved draft: old checks do not transfer across process/snapshot/route identities, and declarations cannot verify invocation. Missing service, Prompt application, MCP connectivity, and policy-enforcement evidence remains unknown; complete A8 and model-service acceptance remain open.

OpenCode ESM plugin bindings are eligible for separate startup checks; pure mode blocks them. Selected Pi extensions require unrestricted execution and separate startup checks; prefix arguments cannot add untracked extensions. Selected DSH modules are eligible for native boot/fiber/session checks and bounded per-instance JSON configuration; options for a different engine are diagnosed before launch. See [DSH activation](dsh-plugin-activation.md). Credential revision reports and selected-file native import are now implemented separately. Complete native provenance/ingestion, broader plugin coverage, external-provider acceptance, and cross-platform acceptance remain open. Windows is blocked by the current desktop runtime path; Linux remains unverified by these macOS fixtures.
