...
- **OpenCode configuration import:** preview a selected native JSON/JSONC file in Settings, then import shared resources and disabled Agent drafts. Known credentials enter the encrypted vault; exact source bytes and unknown fields remain in an encrypted archive with immutable field provenance. [Workflow and boundaries](docs/native-configuration-import.md).
...
| `npm run test:native-import` | Verify OpenCode import, OS encryption, bilingual UI, and an imported native session against a local provider      |
