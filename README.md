...
- **Resource bundles:** reuse prompt, Skill, and MCP bindings. Native plugins have separate metadata tied to an engine installation. The desktop editor can [inspect selected installed OpenCode plugin files](docs/native-plugin-inspection.md), including entry resolution, package versions, and digests; runtime activation remains pending.
