...
export function SessionCapabilities({ report }: { report: SessionCapabilityReport }) {
  ...
  return (
    <details className="session-capabilities">
      ...
      <p className="hint">{t('capability.hint')}</p>
      <p data-testid="capability-current">
        {t(report.current ? 'capability.current' : 'capability.historical')}
      </p>
      <p className="hint">{t('capability.identity')}</p>
      <code className="support-digest">{report.identity.snapshotDigest}</code>
      <code className="support-digest">{report.identity.runId ?? '—'}</code>
